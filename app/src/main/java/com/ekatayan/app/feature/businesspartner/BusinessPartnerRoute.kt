...
fun BusinessPartnerRoute(route: String, listingId: String?, viewModel: BusinessPartnerViewModel,
    ...
    val documents = rememberLauncherForActivityResult(ActivityResultContracts.OpenDocument()) { uri ->
        if (uri == null) viewModel.documentPicked(null) else scope.launch {
            val document = withContext(Dispatchers.IO) {
                runCatching {
                    retain(uri)
                    context.contentResolver.openFileDescriptor(uri, "r")?.use { } ?: error("Unreadable document")
                    val name = context.contentResolver.query(uri, arrayOf(OpenableColumns.DISPLAY_NAME), null, null, null)?.use { cursor ->
                        if (cursor.moveToFirst()) cursor.getString(0) else null
                    } ?: uri.lastPathSegment.orEmpty()
                    BusinessDocument(uri.toString(), name)
                }.getOrNull()
            }
            viewModel.documentPicked(document)
            if (document == null) viewModel.pickerError(true)
        }
    }

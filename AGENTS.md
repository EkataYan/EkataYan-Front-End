...
- Profile uses Retrofit/OkHttp through `data/remote/` and `ProfileRepository` for `GET /api/users/me`, `PATCH /api/users/me`, and authenticated `GET`/`POST /api/storage/profile-picture`. The PATCH request supports `display_name`, `phone`, `bio`, `home_city`, `language`, and `interests`; email is not updated by this contract. Profile pictures are normalized into account-scoped internal app storage, uploaded to the private Supabase `profile-images` bucket, and represented in `profiles.avatar_path`. `EkataYanApiService` centralizes the verified Flask contracts for trips, members, itineraries, expenses, chat, notifications, weather, and storage, but their existing UI repositories remain local until each screen is migrated to asynchronous UUID-backed state. Profile DTOs map into app models; never expose network DTOs to screens.
...
- The profile response has display name, bio, home city, language, interests and an avatar storage path; it does not provide email or travel statistics. Render the account-scoped local avatar when present and use the placeholder only while no avatar is cached or stored.
...
- Do not expose Apple sign-in in the Android Login or Sign Up flows. Email/Supabase password auth and native Google sign-in are the supported public authentication options.
- Google sign-in uses Android Credential Manager plus `googleid`, with the Google Web OAuth Client ID supplied through ignored `local.properties` as `GOOGLE_WEB_CLIENT_ID` and exposed only to the app as `BuildConfig.GOOGLE_WEB_CLIENT_ID`. The Google client secret belongs only in Supabase and must never be added to Android source, BuildConfig, or frontend configuration. The native ID token is exchanged through Supabase Auth's `grant_type=id_token` endpoint and persists through the same encrypted `UserSessionProvider` session path as password auth.
